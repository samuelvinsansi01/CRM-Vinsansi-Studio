...
-- CRM R59 BUILD FIX 23 — PALAVRA-CHAVE NO NOME NA PUXADA
-- Escopo estrito:
-- 1) adiciona filtro opcional p_name_keyword à prévia e à reserva real;
-- 2) mínimo de 3 caracteres quando preenchido;
-- 3) comparação literal, case-insensitive e accent-insensitive somente em leads.leads_name;
-- 4) campo vazio mantém o comportamento atual;
-- 5) não altera captura, validação, aprovação, filas finais, Worker, Gateway ou motores.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'branchIds',to_jsonb(v_branch_ids),'nameKeyword',nullif(v_name_keyword,''),

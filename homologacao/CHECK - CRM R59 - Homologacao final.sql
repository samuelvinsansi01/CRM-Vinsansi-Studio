...
commercial_contract_diff AS (
  SELECT (
    ...
    CASE WHEN (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname IN ('list_leads_page_r59','set_lead_commercial_stage_r59')) <> 2 THEN 1 ELSE 0 END +
    CASE WHEN EXISTS (

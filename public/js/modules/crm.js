...
function updateLeadEverywhereV427(id, patch = {}, options = {}) {
  ...
  if (typeof getLeadBaseData === 'function' && typeof LEADS_BASE_KEY !== 'undefined') {
    ...
    if (result.changed) {
      if (typeof saveOperationalKey === 'function') saveOperationalKey(LEADS_BASE_KEY, result.next, 'crm-permanent-base-patch'); 
...
async function syncLeadCrmMetaToCloudV427(id, reason = 'crm-meta') {
  ...
  uiSyncLog('supabase-save-start', { entity:'lead-crm-meta', id, reason });
  const result = await supabaseDataAdapter.saveLead(payloadLead);
  if (result?.error) {
    uiSyncLog('supabase-save-error', { entity:'lead-crm-meta', id, reason, error:result.error.message || result.error });
    return result;
  }
  uiSyncLog('supabase-save-success', { entity:'lead-crm-meta', id, reason });
  return result;
}
...
async function syncLeadToCloud(id, baseLead = {}) {
  ...
  if (typeof shouldSkipLeadCloudPersistenceV433 === 'function' && shouldSkipLeadCloudPersistenceV433({ ...baseLead, id }, persistenceSource)) {
    uiSyncLog('optimistic-update', { entity:'lead', action:'skip-cloud-persistence', id, reason:'non-persistent-lead' });
    return { skipped:true, reason:'non-persistent-lead' };
  }
  uiSyncLog('supabase-save-start', { entity:'lead', id });
  try {
    ...
    uiSyncLog('supabase-save-success', { entity:'lead', id });
    return { ok:true, result };
  } catch (error) {
    setLeadPersistenceStatusV426(id, 'pending', error?.message || error);
    uiSyncLog('supabase-save-error', { entity:'lead', id, error:error?.message || error });
...
function persistOptimisticLeadV426(lead = {}, action = 'save') {
  ...
  uiSyncLog('optimistic-update', { entity:'lead', action, id:lead.id });
  return syncLeadToCloud(lead.id, lead);
}

async function syncLeadNoteToCloud(id, noteText, baseLead = {}, syncId = '') {
  if (!supabaseDataAdapter || !currentUser || !id) return { skipped:true };
  uiSyncLog('supabase-save-start', { entity:'note', leadId:id, syncId });
  try {
    ...
    if (result?.skipped) {
      ...
      uiSyncLog('optimistic-update', { entity:'note', action:'local-only-until-lead-approved', leadId:id, syncId });
      return result;
    }
    updateLeadNoteSyncStatusV426(id, syncId, 'saved');
    uiSyncLog('supabase-save-success', { entity:'note', leadId:id, syncId });
    return { ok:true, result };
  } catch (error) {
    updateLeadNoteSyncStatusV426(id, syncId, 'pending', error?.message || error);
    uiSyncLog('supabase-save-error', { entity:'note', leadId:id, syncId, error:error?.message || error });
...
async function syncLeadHistoryToCloud(id, eventText, baseLead = {}) {
  if (!supabaseDataAdapter || !currentUser || !id) return { skipped:true };
  uiSyncLog('supabase-save-start', { entity:'history', leadId:id });
  try {
    const result = await supabaseDataAdapter.saveHistory(getLeadForCloud(id, baseLead), eventText);
    if (getSupabaseSaveErrorV426(result)) throw result.error;
    if (result?.skipped) {
      uiSyncLog('optimistic-update', { entity:'history', action:'local-only-until-lead-approved', leadId:id });
      return result;
    }
    uiSyncLog('supabase-save-success', { entity:'history', leadId:id });
    return { ok:true, result };
  } catch (error) {
    uiSyncLog('supabase-save-error', { entity:'history', leadId:id, error:error?.message || error });
...
async function syncLeadFollowUpToCloud(id, dateIso, baseLead = {}) {
  if (!supabaseDataAdapter || !currentUser || !id) return { skipped:true };
  uiSyncLog('supabase-save-start', { entity:'followup', leadId:id });
  try {
    ...
    if (result?.skipped) {
      ...
      uiSyncLog('optimistic-update', { entity:'followup', action:'local-only-until-lead-approved', leadId:id });
      return result;
    }
    setLeadPersistenceStatusV426(id, 'saved');
    uiSyncLog('supabase-save-success', { entity:'followup', leadId:id });
    return { ok:true, result };
  } catch (error) {
    setLeadPersistenceStatusV426(id, 'pending', error?.message || error);
    uiSyncLog('supabase-save-error', { entity:'followup', leadId:id, error:error?.message || error });
...
async function clearLeadFollowUpFromCloud(id, baseLead = {}) {
  if (!supabaseDataAdapter || !currentUser || !id) return { skipped:true };
  uiSyncLog('supabase-save-start', { entity:'followup', action:'clear', leadId:id });
  try {
    const result = await supabaseDataAdapter.clearFollowUp(getLeadForCloud(id, baseLead));
    if (getSupabaseSaveErrorV426(result)) throw result.error;
    setLeadPersistenceStatusV426(id, 'saved');
    uiSyncLog('supabase-save-success', { entity:'followup', action:'clear', leadId:id });
    return { ok:true, result };
  } catch (error) {
    setLeadPersistenceStatusV426(id, 'pending', error?.message || error);
    uiSyncLog('supabase-save-error', { entity:'followup', action:'clear', leadId:id, error:error?.message || error });
...
function addLeadHistory(id, text, baseLead = {}) {
  ...
  saveLeadCrm(id, crm);
  uiSyncLog('optimistic-update', { entity:'history', action:'create', leadId:id });
...
function addLeadNote() {
  ...
  uiSyncLog('optimistic-update', { entity:'note', action:'create', leadId:activeLeadDrawerId, syncId });
...
function updateLeadPipeline(status) {
  ...
  uiSyncLog('optimistic-update', { entity:'lead', action:'pipeline-update', id:activeLeadDrawerId, status });
...
function saveLeadFollowUp() {
  ...
  uiSyncLog('optimistic-update', { entity:'followup', action:'save', leadId:activeLeadDrawerId, dateIso });
...
function clearLeadFollowUp() {
  ...
  crm.followUpDate = '';
  saveLeadCrm(activeLeadDrawerId, crm);
  uiSyncLog('optimistic-update', { entity:'followup', action:'clear', leadId:activeLeadDrawerId });

/* V68 — Reorganização segura da Atribuição sem retirar leads do Instagram
   ...
   - Adiciona botão para reorganizar leads das abas WhatsApp, Com Site e Agregadores.
   - Pode inserir leads no Instagram quando a regra fizer sentido.
   - Nunca retira, lê, processa ou atualiza leads que já estão em attribution_instagram.
   - Preserva aprovações manuais quando o lead continuar sendo Com Site ou Agregador. */
(function(){
  'use strict';
  const VERSION='20260618-v68-reorganizar-atrib-sem-retirar-instagram';
  const USER_ID_FALLBACK='c02fe973-4eb5-4036-9f8d-8787937e8b11';
  const PAGE_SIZE=1000;
  // Regra V68: o reorganizador só pode retirar leads destas abas.
  // Ele pode ENVIAR para attribution_instagram quando o lead tiver Instagram como website,
  // mas nunca consulta/move leads que já estão em attribution_instagram.
  const ATTR_STAGES=[
    'attribution_whatsapp','attribution_site','attribution_aggregator',
  ...
  function ensureReorganizeButton(){
    ...
    if(!header || document.getElementById('v68ReorganizarAtribBtn')) return;
    const wrap=document.createElement('div');
    wrap.style.display='flex'; wrap.style.gap='8px'; wrap.style.alignItems='center';
    wrap.innerHTML=`<button id="v68ReorganizarAtribBtn" class="btn btn-ghost" type="button" style="font-size:10px;padding:8px 12px">↻ Reorganizar leads</button>`;
    header.appendChild(wrap);
    document.getElementById('v68ReorganizarAtribBtn').addEventListener('click',()=>window.reorganizarLeadsAtribuicaoV68());
  }
  function addStyles(){
  ...
  async function fetchAllReorganizableAttributionLeads(){
  ...
  async function insertBasePermanenteForFacebook(leads){
    ...
    const rows=leads.map(l=>({
      ...
      status:'invalidated', invalid_reason:'facebook como website', invalid_source:'reorganizar_atribuicao_v68', invalidated_at:now,
      source:'attribution_reorganizer_v68', raw_payload:l.raw_payload || null, created_at:now, updated_at:now
    }));
    ...
    if(error) console.warn('[v68][base-permanente-facebook]',error.message);
  }

  window.reorganizarLeadsAtribuicaoV68=async function(){
    const c=sb(); if(!c) return notify('// Supabase indisponível','err');
    const ok=confirm('Reorganizar leads das abas WhatsApp, Com Site e Agregadores?\n\nPode inserir leads no Instagram quando fizer sentido.\nNão retira nem altera leads que já estão na aba Instagram.\nNão mexe em Pré-envio, Fila, Disparos ou Conversas.\nFacebook será invalidado e enviado para Base Permanente.');
    if(!ok) return;
    const btn=document.getElementById('v68ReorganizarAtribBtn');
    if(btn){ btn.classList.add('v67-reorg-running'); btn.textContent='reorganizando...'; }
    try{
      const leads=await fetchAllReorganizableAttributionLeads();
      ...
      console.error('[v68][reorganizar]',e); notify('// erro ao reorganizar: '+(e.message||e),'err');
    }finally{
  ...
  window.__V68_REORGANIZAR_ATRIB_SEM_RETIRAR_INSTAGRAM__=VERSION;
})();

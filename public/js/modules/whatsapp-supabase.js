...
function upsertLocalOutgoingWhatsappMessageV426(message = {}, options = {}) {
  ...
  if (options.log !== false) {
    uiSyncLog('optimistic-update', { entity:'message', action:'upsert', id, leadId:localMessage.leadId, status:localMessage.status });
  }
...
function getConversationMetaKeyV421(conversationKey = '') {
  ...
  const instance = String((getConversationMessages(conversationKey)[0]?.instance) || resolved.instance || 'prospecto').trim();
...
function archiveConversationV421(conversationKey = '') {
  if (!conversationKey) conversationKey = activeConversationLead;
...
function getContactMapContextV418(conversationKey = '') {
  const key = conversationKey || activeConversationLead || '';
...
async function associateContactMapLeadV418(leadId = '') {
  ...
  const previousConversationKey = activeConversationLead;
  const optimisticMap = {
  ...
  activeConversationLead = lead.id;
  uiSyncLog('optimistic-update', { entity:'association', action:'save', lid:state.lid, leadId:lead.id, instance:optimisticMap.instance });
  try { renderInbox(); } catch(e) {}
  try { renderConversations(); } catch(e) {}
  ...
  uiSyncLog('supabase-save-start', { entity:'association', lid:state.lid, leadId:lead.id, instance:optimisticMap.instance });
  try {
    ...
    activeConversationLead = lead.id;
    uiSyncLog('supabase-save-success', { entity:'association', lid:state.lid, leadId:lead.id, instance:optimisticMap.instance });
    ...
    activeConversationLead = previousConversationKey;
    uiSyncLog('supabase-save-error', { entity:'association', lid:state.lid, leadId:lead.id, instance:optimisticMap.instance, error:error?.message || error });
...
function findLeadByPhoneV412(phone) {
  try {
    const foundByPhone = typeof findLeadByPhoneV34 === 'function' ? findLeadByPhoneV34(phone) : null;
...
function renderConversationList() {
  ...
  if (!box) return;
  const items = getAllConversationLeads();
  ...
  if (!activeConversationLead) activeConversationLead = items[0].leadId;

  box.innerHTML = items.map(item => {
    const messages = getConversationMessages(item.leadId);
    const last = messages[messages.length - 1];
    const active = item.leadId === activeConversationLead ? 'active' : '';
    ...
    return `
      <div class="conversation-item ${active}" onclick="openConversation('${escHtml(item.leadId)}')">
        <div class="conversation-item-main">
          <div class="conversation-item-top">
            <div class="conversation-item-title">${escHtml(title)}</div>
            <div class="conversation-item-date">${escHtml(dateLabel)}</div>
          </div>
          <div class="conversation-item-preview">${escHtml(preview)}</div>
        </div>
      </div>
    `;
  }).join('');
...
function getAllConversationLeads() {
...
function renderConversationChat() {
  const box = document.getElementById('conversationChat');
  if (!box) return;
  ...
  const resolved = getConversationLeadFromKeyV412(activeConversationLead);
  const lead = resolved.lead || { nome:'Lead' };
  const messages = getConversationMessages(activeConversationLead);
  ...
  const associationButton = lead.isLid && !resolved.leadId
    ? `<button class="btn btn-ghost" onclick="associateLidConversationToLeadV417('${escHtml(activeConversationLead)}')">Associar ao lead</button>`
    : '';
  const canReassociate = lead.isLid || resolved.mappedFromLid || isPhoneConversationKeyV412(activeConversationLead);
  const drawerLeadId = resolved.leadId || (!isPhoneConversationKeyV412(activeConversationLead) ? activeConversationLead : '');
  ...
  box.innerHTML = `
    <div class="conversation-chat-header-v38">
      ...
      <div class="conversation-header-actions-v421">
        ...
        <div id="conversationActionsMenuV421" class="conversation-actions-menu-v421">
          ${drawerLeadId ? `<button onclick="openConversationLeadDrawerV426('${escHtml(activeConversationLead)}')">Abrir ficha do lead</button>` : ''}
          <button onclick="archiveConversationV421('${escHtml(activeConversationLead)}')">Arquivar conversa</button>
          <button onclick="markConversationReadActionV421('${escHtml(activeConversationLead)}')">Marcar como lida</button>
          <button onclick="markConversationUnreadV421('${escHtml(activeConversationLead)}')">Marcar como não lida</button>
          ${canReassociate ? `<button onclick="associateLidConversationToLeadV417('${escHtml(activeConversationLead)}')">Trocar associação de lead</button>` : ''}
          <button onclick="addConversationNoteV421('${escHtml(activeConversationLead)}')">Adicionar nota</button>
          <button onclick="createConversationFollowupV421('${escHtml(activeConversationLead)}')">Criar follow-up</button>
        </div>
      </div>
    </div>
    ...
    <div class="conversation-reply-v38">
      <textarea id="conversationReplyText" placeholder="Responder este lead..." ${canReply ? '' : 'disabled'}></textarea>
      <button class="btn btn-primary" onclick="sendConversationReply()" ${canReply ? '' : 'disabled'}>Responder</button>
    </div>
  `;
}
...
async function persistOutgoingWhatsappMessageV412(message = {}, options = {}) {
  ...
  if (!currentUser?.id || !currentUser?.email || payload.userId !== currentUser.id) {
    ...
    uiSyncLog('supabase-save-error', { entity:'message', id:payload.id, error:'auth-mismatch' });
  ...
  if (!payload.id || !payload.instance || !payload.phone || !payload.text || !payload.userId) {
    ...
    if (queueOnFailure) queuePendingOutgoingWhatsappMessageV412(payload);
    uiSyncLog('supabase-save-error', { entity:'message', id:payload.id, error:'missing-data' });
  ...
  if (apiResult.ok) {
    ...
    uiSyncLog('supabase-save-success', { entity:'message', id:payload.id, via:'api' });
    return apiResult;
  }
  ...
  uiSyncLog('supabase-save-error', { entity:'message', id:payload.id, error:apiResult.error, queued:queueOnFailure });
...
async function sendConversationReply() {
  if (!activeConversationLead) return;
  const resolved = getConversationLeadFromKeyV412(activeConversationLead);
  const lead = resolved.lead || {};
  const realLeadId = resolved.leadId || (!isPhoneConversationKeyV412(activeConversationLead) ? activeConversationLead : '');
  const text = (document.getElementById('conversationReplyText')?.value || '').trim();
  const phone = normalizeWhatsappDigitsV412(resolved.phone || lead.whatsapp || lead.phone || lead.telefone || '');
  const cfg = getConversationEvolutionConfigV412(realLeadId || activeConversationLead);
  ...
  try {
    ...
    persistOutgoingWhatsappMessageV412({
      ...
      uiSyncLog('supabase-save-error', { entity:'message', id:messageId, error:error?.message || error });
...
function renderInbox() {
  ...
  set('inboxUnreadCount', items.reduce((total, item) => total + (item.unreadCount || 0), 0));
  set('inboxWhatsappCount', items.reduce((total, item) => total + (item.totalMessages || 0), 0));
  set('inboxInstagramCount', 0);
    ...
    <div class="inbox-item">
      <div>
        <div class="inbox-title-row">
          <div class="inbox-title">${escHtml(item.lead?.nome || 'Lead')}</div>
          <div class="inbox-count">${item.unreadCount ? escHtml(item.unreadCount + ' novas') : escHtml((item.totalMessages || 0) + ' msgs')}</div>
        </div>
        <div class="inbox-message">${escHtml(item.text || '[mensagem sem texto]')}</div>
        <div class="inbox-meta">${item.pending ? (item.lead?.isLid ? 'contato via LID · ' : 'lead não identificado · ') : ''}${item.lead?.subtitle ? escHtml(item.lead.subtitle) + ' · ' : ''}whatsapp · ${item.at ? escHtml(new Date(item.at).toLocaleString('pt-BR')) : ''}</div>
      </div>
...
function shouldAutoSyncWhatsappMessagesV436() {
  return ['panel-inbox', 'panel-conversations'].some(id => (
    document.getElementById(id)?.classList.contains('active')
  ));
}

function startWhatsappMessagesSyncV412() {
  if (window.__whatsappMessagesSyncV436) return;
  window.__whatsappMessagesSyncV436 = true;
  setTimeout(() => {
    if (shouldAutoSyncWhatsappMessagesV436()) fetchEvolutionResponsesV34({ silent:true });
  }, 2500);
  window.__whatsappMessagesSyncTimerV436 = setInterval(() => {
    if (shouldAutoSyncWhatsappMessagesV436()) fetchEvolutionResponsesV34({ silent:true });
  }, 60000);
}
...
function openConversationFromInboxV41(conversationKey) { activeConversationLead = conversationKey; if (typeof switchPanel === 'function') switchPanel('conversations'); else { try{renderConversations();}catch(e){} } }

/* V66 — Reorganização segura da Atribuição + visual da aba Agregadores
...
(function(){
  'use strict';
  const VERSION='20260618-v66-reorganizar-atrib-agregadores';
  ...
  function desiredStage(lead){
    ...
    const kind=classifySiteValue(lead.website || lead.site || '');
  ...
  function ensureReorganizeButton(){
    ...
    if(!header || document.getElementById('v66ReorganizarAtribBtn')) return;
    const wrap=document.createElement('div');
    wrap.style.display='flex'; wrap.style.gap='8px'; wrap.style.alignItems='center';
    wrap.innerHTML=`<button id="v66ReorganizarAtribBtn" class="btn btn-ghost" type="button" style="font-size:10px;padding:8px 12px">↻ Reorganizar leads</button>`;
    header.appendChild(wrap);
    document.getElementById('v66ReorganizarAtribBtn').addEventListener('click',()=>window.reorganizarLeadsAtribuicaoV66());
  }
  function addStyles(){
    if(document.getElementById('v66-atrib-styles')) return;
    const st=document.createElement('style'); st.id='v66-atrib-styles'; st.textContent=`
      ...
      .v66-reorg-running{opacity:.65;pointer-events:none;}
  ...
  async function fetchAllAttributionLeads(){
    ...
    for(let from=0;;from+=PAGE_SIZE){
      ...
      const {data,error}=await c.from('leads')
        .select('id,company_name,phone,normalized_phone,website,site,instagram_url,instagram,current_stage,lead_type,website_type,has_own_site,city,state,rating,reviews_count,category,category_name,categories,maps_url,raw_payload')
  ...
  async function insertBasePermanenteForFacebook(leads){
    ...
    const rows=leads.map(l=>({
      user_id:uid(), company_name:l.company_name || '', phone:l.phone || '', normalized_phone:l.normalized_phone || '', website:l.website || l.site || '',
      ...
      status:'invalidated', invalid_reason:'facebook como website', invalid_source:'reorganizar_atribuicao_v66', invalidated_at:now,
      source:'attribution_reorganizer_v66', raw_payload:l.raw_payload || null, created_at:now, updated_at:now
    }));
    ...
    if(error) console.warn('[v66][base-permanente-facebook]',error.message);
  }

  window.reorganizarLeadsAtribuicaoV66=async function(){
    ...
    if(!ok) return;
    const btn=document.getElementById('v66ReorganizarAtribBtn');
    if(btn){ btn.classList.add('v66-reorg-running'); btn.textContent='reorganizando...'; }
    try{
      ...
      console.error('[v66][reorganizar]',e); notify('// erro ao reorganizar: '+(e.message||e),'err');
    }finally{
      if(btn){ btn.classList.remove('v66-reorg-running'); btn.textContent='↻ Reorganizar leads'; }
    }
  };
  ...
  window.__V66_REORGANIZAR_ATRIB_AGREGADORES__=VERSION;
})();

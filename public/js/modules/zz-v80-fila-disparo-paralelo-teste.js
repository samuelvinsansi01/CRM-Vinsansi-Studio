...
(function(){
  'use strict';
  const VERSION='20260618-V80-FILA-DISPARO-PARALELO-TESTE';
  ...
  async function sendOne(row,ch,{test=false,testName='Lead teste'}={}){
    ...
    const number=leadPhone(l)||phone(row.phone); if(!number)throw new Error('Telefone ausente');
    if(!test){const cur=await readCurrentStatus(row.id); if(statusKey(cur)==='sent')return {skipped:true,reason:'já enviado'}; await setStatus(row.id,'sending');}
    const {m1,m2}=test?{m1:`Teste de envio da plataforma — Mensagem 1`,m2:`Teste de envio da plataforma — Mensagem 2`} : templatePair(row);
    const r1=await sendText(c,number,m1); await persistOutgoing(row||{},ch,number,m1,r1,'text1');
    await sleep(10000);
    const r2=await sendText(c,number,m2); await persistOutgoing(row||{},ch,number,m2,r2,'text2');
    await sleep(5000);
    let img=null;
    if(test){
    ...
    if(img){const r3=await sendMedia(c,number,img); await persistOutgoing(row||{},ch,number,'[imagem]',r3,'image');}
    if(!test){await setStatus(row.id,'sent'); await markSent(row);}
    return {ok:true};
  }
  ...
  window.startDispatchV80=startDispatchV80; window.pauseDispatchV80=pauseAllV80; window.resumeDispatchV80=resumeAllV80; window.stopDispatchV80=stopAllV80; window.previewDispatchV80=previewV80; window.sendTestDispatchV80=sendTestV80; window.pauseChipV80=pauseChipV80; window.resumeChipV80=resumeChipV80;
  window.__V80_FILA_DISPARO_PARALELO_TESTE__=VERSION;
})();

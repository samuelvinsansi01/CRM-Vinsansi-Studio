...
(function () {
  ...
  function validationRowStatusBadgeV691(row = {}) {
  ...
  function validationLeadPhoneV691(row = {}) {
    return normalizePhoneLabel(row.phone || row.normalized_phone || row.whatsapp || row.telefone || '');
  }

  function validationIconButtonV691(className, title, action, svg) {
    return `<button class="validation-icon-action ${className || ''}" type="button" title="${esc(title)}" aria-label="${esc(title)}" onclick="${action}">${svg}</button>`;
  }

  function validationTableActionButtonsV691(row = {}) {
    const id = esc(row.id);
    const readyForBacklog = isBacklogReadyTab();
    const validated = hasValidatedWhatsapp(row);

    const eyeSvg = `<svg viewBox="0 0 24 24"><path d="M12 5c5.2 0 8.7 4.2 9.8 5.8a2 2 0 0 1 0 2.4C20.7 14.8 17.2 19 12 19s-8.7-4.2-9.8-5.8a2 2 0 0 1 0-2.4C3.3 9.2 6.8 5 12 5Zm0 2C7.8 7 4.8 10.3 3.9 12c.9 1.7 3.9 5 8.1 5s7.2-3.3 8.1-5C19.2 10.3 16.2 7 12 7Zm0 2.5A2.5 2.5 0 1 1 12 14.5 2.5 2.5 0 0 1 12 9.5Z"/></svg>`;
    const checkSvg = `<svg viewBox="0 0 24 24"><path d="M9.2 16.6 4.9 12.3l-1.4 1.4 5.7 5.7L21 7.6l-1.4-1.4-10.4 10.4Z"/></svg>`;
    const archiveSvg = `<svg viewBox="0 0 24 24"><path d="M5 4h14v4H5V4Zm2 6h10v8a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2v-8Zm2 2v6h6v-6H9Z"/></svg>`;

    const ficha = validationIconButtonV691('', 'Ficha', `openValidationLeadDrawerRebuild('${id}')`, eyeSvg);
    const approveTitle = readyForBacklog ? (state.activeTab === 'com-site' ? 'Site ruim / enviar ao backlog' : 'Enviar ao backlog') : (validated ? 'Já validado' : 'Aprovar manualmente');
    const approveAction = readyForBacklog ? `sendValidationLeadToBacklogRebuild629('${id}')` : `approveLeadWhatsappRebuild('${id}')`;
    const approve = validationIconButtonV691('ok', approveTitle, approveAction, checkSvg);
    ...
    const archive = validationIconButtonV691('danger', archiveTitle, archiveAction, archiveSvg);

    return `<div class="validation-table-actions">${ficha}${approve}${archive}</div>`;
  }

  function renderValidationLeadTableRowV691(row) {
    ...
    return `
      <tr class="validation-table-row" data-lead-id="${esc(row.id)}">
        ...
        <td>${validationRowStatusBadgeV691(row)}</td>
        <td>
        ...
        <td>${esc(validationLeadRamoV691(row))}</td>
        <td>${esc(validationLeadStateV691(row))}</td>
        <td>${esc(validationLeadCityV691(row))}</td>
        <td><span class="empresa-phone">${esc(validationLeadPhoneV691(row))}</span></td>
        <td>${validationTableActionButtonsV691(row)}</td>
      </tr>
    `;
  }

  function toggleValidationPageSelectionV691() {
    const ids = Array.isArray(window.__validationVisiblePageIdsV691) ? window.__validationVisiblePageIdsV691 : [];
    if (!ids.length) return;
    const allSelected = ids.every((id) => state.selected.has(String(id)));
    ids.forEach((id) => {
      if (allSelected) state.selected.delete(String(id));
      else state.selected.add(String(id));
    });
    renderActiveValidationTab();
  }
    ...
    list.innerHTML = `
      <div class="validation-table-shell">
        <table class="validation-table">
          <thead>
            <tr>
              <th class="validation-check-cell">
                <button type="button" class="validation-check ${selectedVisible ? 'is-selected' : ''}" title="Selecionar todos desta página" onclick="toggleValidationPageSelectionV691()">
          ...
          <tbody>
            ${pageRows.map(renderValidationLeadTableRowV691).join('')}
          </tbody>
        </table>
      </div>
    `;

    window.__validationVisiblePageIdsV691 = [...pageIds];
  ...
  window.toggleValidationPageSelectionV691 = toggleValidationPageSelectionV691;

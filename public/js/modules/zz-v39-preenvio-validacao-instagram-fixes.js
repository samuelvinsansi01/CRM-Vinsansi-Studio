/* V39 — Estabilização pré-envio + validação WhatsApp + fallback Instagram
   - Corrige consulta da Fila WhatsApp sem depender de nested select que gerava 400.
   - Adiciona validação dos leads do pré-envio pela Evolution.
   - Meta passa a ser por válidos confirmados: repõe inválidos até fechar a quantidade.
   - WhatsApp inválido NÃO vai para backlog Instagram; volta para Atribuição Instagram.
...
(function(){
  ...
  function shortHost(url){ try{return new URL(cleanUrl(url)).hostname.replace(/^www\./,'');}catch(_){return String(url||'').replace(/^https?:\/\/(www\.)?/,'').split('/')[0];} }
  ...
  function leadNameHtml(l){const m=mapsUrl(l);const name=esc(leadName(l));return m?`<a href="${esc(m)}" target="_blank" rel="noopener noreferrer" class="v39-lead-name-link">${name}</a>`:`<span>${name}</span>`;}
  ...
  function currentPreDate(){ const active=document.querySelector('#preWeekCards .pre-day-card.active'); const raw=active?.getAttribute('onclick')||''; const m=raw.match(/setPreEnvioDateV31\(['"]([^'"]+)/); return m?.[1] || todayIso(); }
  function currentPreChip(){ const active=document.querySelector('#preChipTabs .day-tab.active'); const raw=active?.getAttribute('onclick')||''; const m=raw.match(/setPreEnvioChipV31\(['"]([^'"]+)/); return m?.[1] || 'all'; }
  function addPreValidationTools(){ addStyle(); const list=document.getElementById('preEnvioList'); if(!list || document.getElementById('v39PreValidationTools')) return; const box=document.createElement('div'); box.id='v39PreValidationTools'; box.className='v39-pre-tools'; box.innerHTML=`
    <div class="field-group"><label>Meta válida/chip</label><input id="v39ValidateTarget" type="number" min="1" max="500" value="120"></div>
    <input id="v39ValidateDelay" type="hidden" value="10"><div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);padding:0 8px 8px 0">Delay fixo de validação: <b style="color:var(--text)">10s</b></div>
  ...
  async function renderPreCompletionStatusV44(){
    ...
    const target=Math.max(1,Math.min(500,Number(document.getElementById('v39ValidateTarget')?.value||120)));
  ...
  async function completePreChipNowV44(chipInstance){
    ...
    const date=currentPreDate(); const target=Math.max(1,Math.min(500,Number(document.getElementById('v39ValidateTarget')?.value||120))); const delay=10;
  ...
  async function validatePreEnvioDayV39(){ if(state.validating) return notify('Validação já está rodando.','warn'); const c=sb(); if(!c) return notify('Supabase indisponível','err'); const date=currentPreDate(); const selected=currentPreChip(); const target=Math.max(1,Math.min(500,Number(document.getElementById('v39ValidateTarget')?.value||120))); const delay=10; const chips=(await getChips()).filter(ch=>selected==='all'||ch.instance===selected||ch.label===selected); if(!chips.length) return notify('Nenhum chip ativo para validar.','warn'); state.validating=true; document.getElementById('v39ValidateBtn')?.setAttribute('disabled','disabled'); logPre(`Iniciando validação de ${dayLabel(date)} · ${chips.length} chip(s) · meta ${target}/chip`); try{ for(const chip of chips){ const finalValid=await completeChip(date,chip,target,delay); if(finalValid<target) logPre(`${chipTitle(chip)}: incompleto (${finalValid}/${target}). Importe novos leads e clique em Completar agora.`); } await renderPreCompletionStatusV44(); notify('✓ validação concluída'); }catch(e){ console.error('[v39][validate-pre]',e); notify('Erro na validação: '+(e.message||e),'err'); } finally { state.validating=false; document.getElementById('v39ValidateBtn')?.removeAttribute('disabled'); if(typeof window.renderPreEnvioListV31==='function') window.renderPreEnvioListV31(); /* mantém o usuário no Pré-envio; não redireciona para a Fila WhatsApp em erro de validação */ } }

  async function safeFetchFilaData(){ const c=sb(); if(!c) return {items:[],chips:[],error:new Error('Supabase indisponível')}; const statusList=['ready_to_dispatch','queued','dispatch_queue','not_sent','waiting','sent','enviado','responded','respondida','not_responded','nao_respondida','refused','recusada','rejected','closed','fechada','error','erro','no_response']; const itemQ=c.from('pre_dispatch_items').select('id,lead_id,user_id,chip_instance,chip_label,scheduled_date,lead_type,status,position,raw_payload,updated_at').eq('user_id',uid()).in('status',statusList).order('scheduled_date',{ascending:true}).order('chip_label',{ascending:true}).order('position',{ascending:true}); const chipQ=c.from('whatsapp_instances').select('id,chip_id,label,name,instance,base_url,evolution_url,url,api_key,status,connection_state,active,daily_limit,block_size,interval_seconds').eq('user_id',uid()).order('label',{ascending:true}); const [it,ch]=await Promise.all([itemQ,chipQ]); if(it.error) return {items:[],chips:[],error:it.error}; const rows=it.data||[]; const ids=[...new Set(rows.map(r=>r.lead_id).filter(Boolean))]; const leads={}; if(ids.length){ const {data,error}=await c.from('leads').select('*').eq('user_id',uid()).in('id',ids); if(error) console.warn('[v39][fila-leads]',error.message); (data||[]).forEach(l=>leads[l.id]=l); } if(ch.error) console.warn('[v39][fila-chips]',ch.error.message); return {items:rows.map(r=>({...r,lead:leads[r.lead_id]||{}})),chips:(ch.data||[]).filter(x=>x.active!==false && x.instance),error:null}; }
  ...
  function countStatus(st, rows){ return st==='all'?rows.length:rows.filter(r=>statusKey(r.status)===st).length; }
  async function setZapStatus(id,st){ const c=sb(); if(!c) return; const db=statusDb(st); const {data:item}=await c.from('pre_dispatch_items').select('id,lead_id').eq('user_id',uid()).eq('id',id).maybeSingle(); const {error}=await c.from('pre_dispatch_items').update({status:db,updated_at:now()}).eq('user_id',uid()).eq('id',id); if(error) return notify('Erro ao atualizar: '+error.message,'err'); if(item?.lead_id){ const patch=db==='sent'?{current_stage:'sent',current_status:'sent',status:'sent',updated_at:now()}:db==='queued'?{current_stage:'dispatch_queue',current_status:'queued',status:'queued',updated_at:now()}:{updated_at:now()}; await c.from('leads').update(patch).eq('user_id',uid()).eq('id',item.lead_id); if(db==='sent'&&typeof window.recordContactEventV38==='function'){ const lead=(state.fila.lastData?.items||[]).find(r=>r.id===id)?.lead||{}; await window.recordContactEventV38({lead,lead_id:item.lead_id,channel:'whatsapp',source_account:'fila_whatsapp',event_type:'sent',status:'sent',sent_at:now(),metadata:{pre_dispatch_item_id:id,origem:'v39_fila'}}); } } await renderFilaZapV39(); }
  function renderCard(r,compact=false){ const l=r.lead||{}; const phone=leadPhone(l); const web=String(l.website||'').trim(); return `<div class="v39-card ${compact?'compact':''}" data-id="${esc(r.id)}" data-lead-id="${esc(r.lead_id||l.id||'')}"><div class="v39-main"><div class="v39-lead-name">${leadNameHtml(l)}</div><div class="v39-meta">${web?`<a href="${esc(cleanUrl(web))}" target="_blank" rel="noopener">${esc(shortHost(web))}</a>`:'<span>Sem site</span>'}<span>|</span><button class="zap" onclick="copyV39('${esc(phone)}')">WhatsApp</button><span>${esc(rowChipName(r))}</span>${l.city||l.state?`<span>${esc([l.city,l.state].filter(Boolean).join('/'))}</span>`:''}${l.rating?`<span>⭐ ${esc(l.rating)} · ${esc(l.reviews_count||0)}</span>`:''}</div><div class="v39-dispatch-info"><span><strong>Lote:</strong> ${loteNum(r)}</span><span><strong>Fluxo:</strong> Msg1 + Msg2 + Imagem</span><span><strong>Tipo:</strong> ${web?'com site':'sem site'}</span><span><strong>Ramo:</strong> ${esc(l.parent_category||l.category_name||l.category||r.lead_type||'não definido')}</span></div></div><div class="v39-actions"><button class="v39-btn" onclick="openLeadDrawer('${esc(r.lead_id||l.id||'')}')">Ficha</button><span class="v39-status ${statusKey(r.status)}">${esc(statusLabel(r.status))}</span><button class="v39-btn" onclick="copyV39('${esc(phone)}')">Copiar nº</button><button class="v39-btn" onclick="openWaV39('${esc(phone)}')">Abrir WA</button><button class="v39-btn blue" onclick="setZapStatusV39('${esc(r.id)}','queued')">Em fila</button><button class="v39-btn" onclick="setZapStatusV39('${esc(r.id)}','sent')">Enviada</button><button class="v39-btn danger" onclick="setZapStatusV39('${esc(r.id)}','error')">Erro</button></div></div>`; }
  async function renderFilaZapV39(){ addStyle(); const panel=document.getElementById('panel-fila-zap'); if(!panel) return; document.querySelectorAll('.panel').forEach(p=>{const on=p.id==='panel-fila-zap';p.classList.toggle('active',on);p.style.display=on?'flex':'none';}); panel.classList.add('v39-panel'); panel.innerHTML=`<div class="v39-left"><div class="page-header" style="padding:22px 20px 0"><div class="page-title">Fila <span>WhatsApp.</span></div><div class="page-sub">// carregando fila operacional...</div></div></div><div class="v39-right"><div class="v39-empty">// carregando chips...</div></div>`; const data=await safeFetchFilaData(); state.fila.lastData=data; if(data.error){ panel.innerHTML=`<div class="page-header"><div class="page-title">Fila <span>WhatsApp.</span></div><div class="page-sub" style="color:var(--error)">// erro: ${esc(data.error.message)}</div></div>`; return; } const rows=data.items||[]; const dates=[...new Set([...weekDates(),...rows.map(r=>r.scheduled_date).filter(Boolean)])].sort(); if(!state.fila.date||!dates.includes(state.fila.date)) state.fila.date=dates.includes(todayIso())?todayIso():(dates[0]||todayIso()); const byDate=rows.filter(r=>r.scheduled_date===state.fila.date); const chips=[...(data.chips||[])]; rows.forEach(r=>{ if(!chips.some(ch=>chipKey(ch)===rowChipKey(r)||chipTitle(ch)===rowChipName(r))) chips.push({id:rowChipKey(r)||rowChipName(r),instance:rowChipKey(r),label:rowChipName(r),daily_limit:120,active:true}); }); const selected=selectedRows(); const statuses=['all','ready','queued','sent','responded','no_response','rejected','closed','error']; const left=`<div class="v39-left"><div class="page-header" style="padding:22px 20px 0;flex-shrink:0"><div class="page-title">Fila <span>WhatsApp.</span></div><div class="page-sub">// ${rows.length} lead(s) na fila final · sem erro 400 · dados carregados em duas etapas</div></div><div class="v39-body"><div class="v39-tabs">${dates.map(d=>`<button class="v39-tab ${state.fila.date===d?'active':''}" onclick="setFilaZapDateV39('${esc(d)}')">${esc(dayLabel(d))} (${rows.filter(r=>r.scheduled_date===d).length})</button>`).join('')}</div><div class="v39-tabs">${statuses.map(st=>`<button class="v39-tab ${state.fila.status===st?'active':''}" onclick="setFilaZapStatusV39('${esc(st)}')">${st==='all'?'Todos':esc(statusLabel(st))} (${countStatus(st,byDate)})</button>`).join('')}</div><div style="display:flex;gap:10px;align-items:center;margin:2px 0 12px;font-family:'DM Mono',monospace;font-size:10px;color:var(--muted)"><span><b style="color:var(--text)">${selected.length}</b> exibindo</span><button class="v39-btn primary" onclick="alert('Disparo automático seguirá na próxima etapa. Agora a fila está estável para status e validação.')">Iniciar disparo</button></div><div class="v39-list">${selected.length?selected.map(r=>renderCard(r)).join(''):'<div class="v39-empty">// nenhuma empresa neste filtro</div>'}</div></div></div>`; const right=`<div class="v39-right"><div style="padding:14px 16px 8px;font-family:'DM Mono',monospace;font-size:10px;color:var(--muted);border-bottom:1px solid var(--border)">Chips ativos · clique para filtrar</div><div class="v39-chip"><div class="v39-chip-head" onclick="setFilaZapChipV39('all')"><div><div class="v39-chip-title">Todos</div><div class="v39-chip-sub">visão geral</div></div><div class="v39-chip-count">${byDate.length}</div></div></div>${chips.map((ch,idx)=>{const key=chipKey(ch),title=chipTitle(ch);const list=byDate.filter(r=>rowChipKey(r)===key||rowChipName(r)===title||rowChipKey(r)===title||rowChipName(r)===key);const open=state.fila.chip===key||state.fila.chip===title||(state.fila.chip==='all'&&idx===0);return `<div class="v39-chip ${open?'open':''}"><div class="v39-chip-head" onclick="setFilaZapChipV39('${esc(open?'all':key)}')"><div><div class="v39-chip-title">${esc(title)}</div><div class="v39-chip-sub">${esc(ch.name||ch.instance||ch.chip_id||'')}</div></div><div class="v39-chip-count">${list.length}/${Number(ch.daily_limit||120)||120}</div></div><div class="v39-chip-body">${list.length?list.map(r=>renderCard(r,true)).join(''):'<div class="v39-empty">// nenhum lead neste chip</div>'}</div></div>`;}).join('')}</div>`; panel.innerHTML=left+right; const badge=document.getElementById('badge-fila-zap'); if(badge) badge.textContent=String(rows.filter(r=>['ready','queued'].includes(statusKey(r.status))).length); }
  function setDate(d){state.fila.date=d;renderFilaZapV39();} function setStatus(s){state.fila.status=s;renderFilaZapV39();} function setChip(c){state.fila.chip=c||'all';renderFilaZapV39();}
  function copy(text){const t=String(text||'');if(!t)return;navigator.clipboard?.writeText(t).then(()=>notify('✓ copiado')).catch(()=>{});} function openWa(phone){const p=normPhone(phone);if(p)window.open(`https://wa.me/${p}`,'_blank','noopener,noreferrer');}
  ...
  window.setFilaZapChipV39=setChip; window.setFilaZapChipV33=setChip; window.setFilaZapChipV31=setChip;
  window.setZapStatusV39=setZapStatus; window.setZapStatusV33=setZapStatus; window.setFilaZapItemStatusV32=setZapStatus;
  window.copyV39=copy; window.openWaV39=openWa;
  const prevSwitch=window.switchPanel;
  window.switchPanel=function(name){const n=String(name||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,''); if(['whatsapp','fila-zap','fila_whatsapp','zap'].includes(n)||name==='WhatsApp'){renderFilaZapV39();return;} return typeof prevSwitch==='function'?prevSwitch(name):undefined;};
  document.addEventListener('click',function(e){const nav=e.target.closest?.('.nav-item[data-label]'); if(!nav) return; if((nav.getAttribute('data-label')||'')!=='WhatsApp') return; e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation)e.stopImmediatePropagation(); renderFilaZapV39();},true);
  document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{if(document.getElementById('panel-fila-zap')?.classList.contains('active')) renderFilaZapV39();}catch(_){}},500));
  window.__V39_PREENVIO_VALIDACAO__=VERSION;
})();

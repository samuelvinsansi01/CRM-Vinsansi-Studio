...
(function(){
  ...
  async function completePreChipNowV44(chipInstance){
    ...
    const date=currentPreDate(); const target=Math.max(1,Math.min(500,Number(document.getElementById('v39ValidateTarget')?.value||120))); const delay=Math.max(0,Math.min(300,Number(document.getElementById('v39ValidateDelay')?.value||30)));
  ...
  async function validatePreEnvioDayV39(){ if(state.validating) return notify('Validação já está rodando.','warn'); const c=sb(); if(!c) return notify('Supabase indisponível','err'); const date=currentPreDate(); const selected=currentPreChip(); const target=Math.max(1,Math.min(500,Number(document.getElementById('v39ValidateTarget')?.value||120))); const delay=Math.max(0,Math.min(300,Number(document.getElementById('v39ValidateDelay')?.value||30))); const chips=(await getChips()).filter(ch=>selected==='all'||ch.instance===selected||ch.label===selected); if(!chips.length) return notify('Nenhum chip ativo para validar.','warn'); state.validating=true; document.getElementById('v39ValidateBtn')?.setAttribute('disabled','disabled'); logPre(`Iniciando validação de ${dayLabel(date)} · ${chips.length} chip(s) · meta ${target}/chip`); try{ for(const chip of chips){ const finalValid=await completeChip(date,chip,target,delay); if(finalValid<target) logPre(`${chipTitle(chip)}: incompleto (${finalValid}/${target}). Importe novos leads e clique em Completar agora.`); } await renderPreCompletionStatusV44(); notify('✓ validação concluída'); }catch(e){ console.error('[v39][validate-pre]',e); notify('Erro na validação: '+(e.message||e),'err'); } finally { state.validating=false; document.getElementById('v39ValidateBtn')?.removeAttribute('disabled'); if(typeof window.renderPreEnvioListV31==='function') window.renderPreEnvioListV31(); /* mantém o usuário no Pré-envio; não redireciona para a Fila WhatsApp em erro de validação */ } }

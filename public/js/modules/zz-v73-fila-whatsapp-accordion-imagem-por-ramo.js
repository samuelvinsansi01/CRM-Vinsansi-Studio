/* V73 — Fila WhatsApp: accordion por chip/lote/lead + imagem por ramo
...
(function(){
  'use strict';
  const VERSION='20260618-V73-FILA-ACCORDION-IMAGEM-POR-RAMO';
  const USER_ID_FALLBACK='c02fe973-4eb5-4036-9f8d-8787937e8b11';
  const state={date:null,status:'all',chipOpen:null,lotes:{},expanded:{},last:null,imgCache:{}};
  ...
  function statusKey(s){
    ...
    if(['ready_to_dispatch','not_sent','waiting','review','approved','nao_enviada','pending','scheduled'].includes(raw))return 'ready';
    if(['queued','em_fila','dispatch_queue'].includes(raw))return 'queued';
    if(['sending','enviando'].includes(raw))return 'sending';
    if(['sent','enviado','enviada'].includes(raw))return 'sent';
    if(['paused','pausado','pausada'].includes(raw))return 'paused';
    if(['error','erro','failed','dispatch_error'].includes(raw))return 'error';
    return 'ready';
  }
  function statusLabel(s){return ({ready:'Não enviada',queued:'Pendente',sending:'Enviando',sent:'Enviada',paused:'Pausada',error:'Erro'})[statusKey(s)]||'Não enviada';}
  function statusDb(s){return ({ready:'ready_to_dispatch',queued:'queued',sending:'sending',sent:'sent',paused:'paused',error:'error'})[s]||s;}

  function blockSize(ch){
    const fromChip=Number(ch?.block_size||0);
    if(fromChip>0)return fromChip;
    try{if(typeof window.getLoteSize==='function')return Math.max(1,Number(window.getLoteSize())||60);}catch(_){}
    const el=Number(document.getElementById('loteTamanho')?.value||0);
    return el>0?el:60;
  }
  ...
  function addStyle(){if(document.getElementById('v73-fila-style'))return; const st=document.createElement('style'); st.id='v73-fila-style'; st.textContent=`
    ...
    #panel-fila-zap .v73-badge.ready{color:var(--accent);border-color:rgba(184,240,89,.38);}
    #panel-fila-zap .v73-badge.queued,#panel-fila-zap .v73-badge.sending{color:#4ab3ff;border-color:rgba(74,179,255,.38);}
  ...
  function statusTabs(rows){const sts=['all','ready','queued','sending','sent','paused','error']; return sts.map(st=>{const cnt=st==='all'?rows.length:rows.filter(r=>statusKey(r.status)===st).length; const label=st==='all'?'Todos':statusLabel(st); return `<button class="status-tab ${state.status===st?'active':''}" onclick="setFilaZapStatusV73('${esc(st)}')">${esc(label)} <span class="st-count">${cnt}</span></button>`;}).join('');}
  ...
  function renderLotes(list,ch){
    ...
    for(let i=0;i<list.length;i+=size){
      ...
      html+=`<div class="v73-lote ${open?'open':''}"><div class="v73-lote-head" onclick="toggleLoteV73('${esc(loteId)}')"><span class="v73-chev">▶</span><span class="v73-lote-title">LOTE ${n} — #${i+1}–${i+lote.length}</span><span class="v73-lote-line"></span><span class="v73-lote-meta">${lote.length} leads · ${sent} enviados · ${pending} pendentes · ${err} erros · ${ramos.length} ramo${ramos.length!==1?'s':''} · ${complete?'✓ completo':`${size-lote.length} restantes`}</span></div><div class="v73-lote-body">${ramoSlotsHtml(ch,n,lote)}${lote.map((r,j)=>renderLead(r,i+j+1,ch,n)).join('')}</div></div>`;
    }
    return html;
  }
  ...
  window.__V73_FILA_WHATSAPP_ACCORDION_IMAGEM_RAMO__=VERSION;
})();

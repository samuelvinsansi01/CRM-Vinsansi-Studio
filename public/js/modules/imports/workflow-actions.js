...
function updateWhatsappInLocalCollectionsV43(id, phone) {
  ...
  try {
    ...
    if (Array.isArray(base) && base.some(lead => String(lead?.id) === String(id))) {
      ...
      if (typeof saveOperationalKey === 'function') saveOperationalKey(LEADS_BASE_KEY, patched, 'whatsapp-lead-base-patch'); 
    }
  ...
  try {
    ...
    if (touched) { if (typeof saveAcompData === 'function') saveAcompData(acomp);  }
...
async function persistWhatsappLeadUpdateV43(lead) {
  ...
  try {
    ...
    if (window.supabaseDataAdapter?.saveLead) {
      uiSyncLog('supabase-save-start', { entity:'lead', id:lead.id, action:'whatsapp-update' });
      const result = await window.supabaseDataAdapter.saveLead(lead);
      if (result?.error) {
        uiSyncLog('supabase-save-error', { entity:'lead', id:lead.id, action:'whatsapp-update', error:result.error?.message || result.error });
      } else {
        uiSyncLog('supabase-save-success', { entity:'lead', id:lead.id, action:'whatsapp-update' });
      }
...
function saveWhatsapp(id, day) {
  ...
  uiSyncLog('optimistic-update', { entity:'lead', action:'whatsapp-update', id, phone });
  ...
  persistWhatsappLeadUpdateV43(emp).then(result => {
    ...
    uiSyncLog('supabase-save-error', { entity:'lead', action:'whatsapp-update', id, error:error?.message || error });
    notify('Número atualizado na tela. Salvamento no Supabase pendente.', 'warn');
  });

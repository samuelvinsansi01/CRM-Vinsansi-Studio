...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM public.platform_tools WHERE tool_id='vinsansi_instagram' AND latest_version='2.0.2') INTO v_ok; IF NOT v_ok THEN v_missing:=array_append(v_missing,'row:platform_tools:tool_id=vinsansi_instagram AND latest_version=2.0.1');END IF;
  IF cardinality(v_missing)>0 THEN RAISE EXCEPTION 'TESTE_FALHOU:%',array_to_string(v_missing,',');END IF;

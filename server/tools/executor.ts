...
export async function effectiveConfig(client: SupabaseClient, organizationId: number, id: ToolId) {
  ...
  return {toolId:id,organizationId,settings:settings.data?.settings??tool.data.default_settings,entitlements:entitlements.data?.entitlements??tool.data.default_entitlements,contracts:id==='vinsansi_capture'?{acquisitionTargets:{source:'branches.branches_categories',whatsappCapacityField:'stockTargetWhatsapp',instagramCapacityField:'stockTargetInstagram',strategy:'additive_execution_target',usefulStockMeasurement:'not_available'}}:undefined,settingsVersion:Number(settings.data?.settings_version??0),entitlementsVersion:Number(entitlements.data?.entitlements_version??0),settingsSchemaVersion:Number(tool.data.settings_schema_version),generatedAt:new Date().toISOString()};
}

...
export default async function handler(req:Stage5Request,res:Stage5Response){
  ...
  try{
    ...
    // Uma repetição idempotente em estado PENDING é retomável: o provider ainda
    // não foi chamado, então devolvemos novamente o comando para o outbox local.
    // Estados sending/reconciliation/sent nunca voltam ao provider automaticamente.
    if(prepared.idempotent===true&&currentStatus!=='pending')return send(res,['sending','reconciliation_required'].includes(currentStatus)?202:200,{ok:true,prepared,command:null});
    ...
    return send(res,prepared.idempotent===true?202:200,{ok:true,prepared,command:{...command,recipient},resumeSafe:prepared.idempotent===true&&currentStatus==='pending'});

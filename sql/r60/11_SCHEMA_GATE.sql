...
BEGIN
 ...
 WHERE ns.nspname='public' AND (
   ...
   p.proname IN('service_ingest_evolution_message','service_update_evolution_message_status','service_update_evolution_connection_state_r60','service_upsert_evolution_chat','service_stage5_converge_automatic_message','service_arm_messaging_resume_gate_r60','service_exchange_executor_pairing','service_cleanup_pairings_r60','service_retention_r60','service_expire_awaiting_response_leads_r60','service_register_release_candidate_r60','service_mark_release_signature_verified_r60','service_mark_release_candidate_homologated_r60','service_promote_release_candidate_r60')
 ...
 END IF;
 RETURN jsonb_build_object('ok',true,'schema','r60','securityBaseline','r60-security-baseline-v1','releaseSequence',60,'machineAcl','closed','forceRls',true,'pairing','r60','retention','r60','presence','realtime','commercialAutomation','72h-no-response');
END $$;

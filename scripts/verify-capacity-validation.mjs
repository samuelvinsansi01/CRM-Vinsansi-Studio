...
const assertions = [
  ...
  [service.includes('validationSlots = Math.max(0, validationSlots - validation.approved)'), 'inválidos estão consumindo vaga de validação'],
  [service.includes("queuePreparationService.enqueueValidated('WhatsApp'"), 'aprovados não são inseridos na fila do chip selecionado'],
...
console.log('Validação por capacidade: chip ativo, limite diário, reposição de inválidos e fila automática confirmados.');

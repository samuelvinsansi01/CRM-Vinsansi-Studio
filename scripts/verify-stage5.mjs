...
for(const file of ['PATCH-ETAPA-5-FASE-A-CONVERSAS.sql','PATCH-CORRETIVO-ETAPA-5-UNIFICAR-CONVERSAS.sql','APLICAR-NO-SUPABASE-v1.4.0.sql','supabase/migrations/20260823120000_whatsapp_manager_conversations.sql','supabase/migrations/20260823170000_whatsapp_conversation_identity_aliases.sql','server/whatsapp/stage5.ts','server/routes/whatsapp/conversations.ts','server/routes/whatsapp/conversation-messages.ts','server/routes/whatsapp/conversation-action.ts','server/routes/whatsapp/conversation-presence.ts','server/routes/whatsapp/manual-message.ts','server/routes/whatsapp/conversation-media.ts','server/routes/whatsapp/queue-operations.ts'])expect(exists(file),`${file} missing`);
...
const identityPatch=read('PATCH-CORRETIVO-ETAPA-5-UNIFICAR-CONVERSAS.sql');
const identityMigration=read('supabase/migrations/20260823170000_whatsapp_conversation_identity_aliases.sql');
expect(identityPatch===identityMigration,'Stage5 conversation identity patch diverges from migration');
for(const token of ['conversation_contact_aliases','stage5_preferred_contact_jid','stage5_merge_conversations','stage5_register_conversation_aliases','stage5_resolve_conversation_id','remoteJidAlt','external_message_id e a evidencia'])expect(identityPatch.includes(token),`Conversation identity hotfix missing ${token}`);
const consolidated=read('APLICAR-NO-SUPABASE-v1.4.0.sql');for(const token of ['CRM Vinsansi Studio v1.4.0','service_stage5_prepare_manual_message','service_stage5_reconcile_queue_item','conversation-media','conversation_contact_aliases','stage5_merge_conversations'])expect(consolidated.includes(token),`Consolidated v1.4.0 missing ${token}`);

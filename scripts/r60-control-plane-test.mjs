...
pass('candidate identity and stage',()=>{assert.equal(manifest.release,'2.4.0-R60.24');assert.equal(manifest.releaseStage,'candidate');assert.equal(manifest.releaseSequence,60);});
pass('candidate is closed',()=>{assert.equal(manifest.productionReady,false);assert.equal(manifest.resumeAllowed,false);});
pass('candidate versions projected correctly',()=>{assert.equal(manifest.manager.latestVersion,'1.5.88');assert.equal(manifest.components.worker.version,'3.14.8');assert.equal(manifest.components.gateway.version,'1.2.34');assert.equal(manifest.components.evolution.version,'0.7.2');assert.equal(manifest.capture.latestVersion,'1.0.53');assert.equal(manifest.instagram.latestVersion,'2.0.8');});

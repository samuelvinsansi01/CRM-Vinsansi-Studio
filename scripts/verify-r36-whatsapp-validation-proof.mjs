...
const checks = [
  ...
  ['revalidação restaura PRE_SEND antes da aprovação', review.includes('await reconcileWhatsApp(item.batchId, validation.approvedIds, retryable)') || (review.indexOf('await restoreWhatsAppValid(item.batchId, validation.approvedIds)') < review.indexOf('await prune(item.batchId).catch'))],
];

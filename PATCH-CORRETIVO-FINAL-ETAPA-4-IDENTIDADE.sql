...
-- Patch incremental para bancos que já receberam o v1.3.0. Mantém as
-- assinaturas e passa a expor o nome canônico de public.users pela cadeia
-- auth.users -> public.users -> organization_members.
CREATE OR REPLACE FUNCTION public.service_executor_member_context(p_auth_users_id uuid,p_organizations_id bigint,p_tool_id text)
...
BEGIN
  ...
  SELECT * INTO v_member FROM public.organization_members WHERE organizations_id=p_organizations_id AND users_id=v_user AND status_id=1;
  ...
  ELSE
    SELECT coalesce(array_agg(DISTINCT p.permissions_key ORDER BY p.permissions_key),'{}') INTO v_permissions FROM public.organization_role_permissions rp JOIN public.permissions p ON p.permissions_id=rp.permissions_id WHERE rp.organization_roles_id=v_member.organization_roles_id AND p.permissions_sensitivity='delegable';
  ...
  RETURN jsonb_build_object('authUserId',p_auth_users_id,'userId',v_user,'organizationId',v_org.organizations_id,'organizationName',v_org.organizations_name,'legacyScopeUsersId',v_org.legacy_scope_users_id,'memberId',v_member.organization_members_id,'memberName',v_member_name,'accessLevel',v_member.access_level,'membershipStatusId',v_member.status_id,'permissions',to_jsonb(v_permissions),'requiredPermission',v_required);
END;
$$;
...
BEGIN
  IF auth.role()<>'service_role' THEN RAISE EXCEPTION 'service_role_required'; END IF;
  SELECT users_id,coalesce(nullif(trim(users_name),''),'Usuário') INTO v_user,v_member_name FROM public.users WHERE auth_user_id=p_auth_users_id AND coalesce(users_is_scope,false)=false;
  SELECT coalesce(jsonb_agg(jsonb_build_object('organizationId',m.organizations_id,'organizationName',o.organizations_name,'memberId',m.organization_members_id,'memberName',v_member_name,'accessLevel',m.access_level) ORDER BY o.organizations_name),'[]'::jsonb) INTO v_result
   ...
   WHERE m.users_id=v_user AND m.status_id=1
     AND (m.access_level='owner' OR EXISTS(SELECT 1 FROM public.organization_role_permissions rp JOIN public.permissions p ON p.permissions_id=rp.permissions_id WHERE rp.organization_roles_id=m.organization_roles_id AND p.permissions_key=CASE p_tool_id WHEN 'vinsansi_capture' THEN 'capture.use' WHEN 'vinsansi_instagram' THEN 'instagram.use' ELSE 'whatsapp.view' END));
  RETURN v_result;
END;
$$;

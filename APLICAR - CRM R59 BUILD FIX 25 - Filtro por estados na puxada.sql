...
-- CRM R59 BUILD FIX 25 — FILTRO POR ESTADOS NA PUXADA
-- Escopo estrito:
-- 1) adiciona filtro opcional p_state_ids à prévia e à reserva real;
-- 2) nenhum estado selecionado mantém o comportamento atual (todos os estados);
-- 3) múltiplos estados usam leads.states_id = ANY(p_state_ids);
-- 4) Preview e Puxar usam exatamente o mesmo filtro;
-- 5) preserva os filtros existentes de ramo, nome, site e Instagram;
-- 6) não altera captura, validação, aprovação, filas finais, Worker, Gateway ou motores.
...
DROP FUNCTION IF EXISTS public.pull_queue_review_to_capacity(text,text,date,text,text,bigint[],text);
DROP FUNCTION IF EXISTS public.pull_queue_review_to_capacity(text,text,date,text,text,bigint[],text,bigint[]);
CREATE FUNCTION public.pull_queue_review_to_capacity(
  ...
  p_state_ids bigint[] DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_state_ids bigint[]:=coalesce(p_state_ids,'{}'::bigint[]);
  v_name_keyword text:=regexp_replace(lower(public.unaccent(btrim(coalesce(p_name_keyword,'')))), '\s+', ' ', 'g');
  v_local_today date;
...
BEGIN
  ...
  IF length(v_name_keyword) IN (1,2) THEN RAISE EXCEPTION 'queue_review_name_keyword_min_3'; END IF;
  ...
  IF v_wanted>0 AND v_channel='whatsapp' THEN
    FOR v_lead IN
      ...
      WHERE l.organizations_id=v_org AND l.users_id=v_user
        ...
        AND (coalesce(array_length(v_state_ids,1),0)=0 OR l.states_id=ANY(v_state_ids))
        AND (v_name_keyword='' OR strpos(regexp_replace(lower(public.unaccent(btrim(coalesce(l.leads_name,'')))), '\s+', ' ', 'g'),v_name_keyword)>0)
  ...
  RETURN jsonb_build_object(
    ...
    'branchIds',to_jsonb(v_branch_ids),'stateIds',to_jsonb(v_state_ids),'nameKeyword',nullif(v_name_keyword,''),
...
DROP FUNCTION IF EXISTS public.preview_queue_review_pull(text,text,date,text,text,bigint[],text);
DROP FUNCTION IF EXISTS public.preview_queue_review_pull(text,text,date,text,text,bigint[],text,bigint[]);

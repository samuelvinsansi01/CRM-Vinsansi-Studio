...
function lotCell(lead:FinalLead){const number=Math.max(1,Number(lead.dispatch_batch_number??lead.batch_number??1));const count=Math.max(number,Number(lead.dispatch_batch_count??1));return <Tag tone="primary">Lote {number}/{count}</Tag>;} 
...
export function QueueFinalTable({ channel, leads, total, page, rowsPerPage, refreshing=false, resourceLabel, canEdit, canInvalidate, onPageChange, onRowsPerPageChange, onView, onEdit, onInvalidate }: {
  ...
  const rows=useMemo<FinalRow[]>(()=>displayLeads.map((lead)=>({ id:lead.id, position:lead.position, lot:lotCell(lead), lotStart:Number(lead.dispatch_batch_position??0)===1, company:company(lead), branch:lead.branch||'—', state:lead.state||'—', city:lead.city||'—', rating:Number(lead.rating||0).toFixed(1), reviews:Number(lead.reviews||0).toLocaleString('pt-BR'), channel:channelCell(lead), instagram:availability(Boolean(String(lead.instagram_username||lead.instagram_url||lead.instagram||'').trim()),instagramHref(lead.instagram_username||lead.instagram_url||lead.instagram||''),'Abrir Instagram'), site:availability(Boolean(String(lead.site||'').trim()),externalHttpHref(lead.site),'Abrir site'), status:queueStatus(lead) })),[displayLeads]);
  const columns=useMemo<TableColumn<FinalRow>[]>(()=>{
    const base:TableColumn<FinalRow>[]=[
      {key:'position',label:'#',width:'4%'},{key:'lot',label:'Lote',width:'8%'},{key:'company',label:'Empresa',width:'18%'},{key:'branch',label:'Ramo',width:'10%'},{key:'state',label:'Estado',width:'5%'},{key:'city',label:'Cidade',width:'9%'},{key:'rating',label:'Nota',width:'5%'},{key:'reviews',label:'Avaliações',width:'7%'},{key:'channel',label:channel,width:'8%'},
    ];
  ...
  return <TableCard title={`Listagem de disparos · ${resourceLabel}`} footerText={`${refreshing?'Atualizando · ':''}Mostrando ${rows.length} de ${total} lead(s)`} footerLeft={total?<RowsPerPageControl value={rowsPerPage} onChange={onRowsPerPageChange}/>:undefined} page={page} totalPages={totalPages} onPageChange={onPageChange}>
    ...
    {rows.length?<DataTable columns={columns} rows={rows} selectable={false} actions={['view','edit','invalidate']}
      getRowClassName={(row,index)=>row.lotStart===true&&(offset+index)>0?'data-table__row--group-start':''}

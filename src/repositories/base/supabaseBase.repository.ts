...
function map(row: Row): BaseLead {
  ...
  return {
    ...
    suppressed:Boolean(row.is_suppressed), commercialOutcome:String(row.commercial_outcome??''), operatorNotes:String(row.operator_notes??''),
  };
}

const SELECT=`permanent_records_id,users_id,canonical_lead_id,branches_id,states_id,cities_id,channels_id,last_lead_status_id,company_name,normalized_phone,normalized_instagram,normalized_domain,normalized_maps,total_leads,total_dispatches,first_seen_at,last_activity_at,last_sent_at,is_suppressed,commercial_outcome,operator_notes,record_status,archived_at,branches:branches_id(branches_name),states:states_id(states_name,states_code),cities:cities_id(cities_name),channels:channels_id(channels_name)`;
...
function filtered(records:BaseLead[],filters:BaseFilters={}) { const q=normalize(filters.search); return records.filter((lead)=>(!q||normalize(`${lead.company} ${lead.phone} ${lead.instagram} ${lead.site} ${lead.city} ${lead.state} ${lead.branch}`).includes(q))&&(!filters.origin||filters.origin==='Todos'||lead.origin===filters.origin)&&(!filters.branch||filters.branch==='Todos'||lead.branch===filters.branch)&&(!filters.state||filters.state==='Todos'||normalize(lead.state)===normalize(filters.state))&&(!filters.city||filters.city==='Todos'||lead.city===filters.city)&&(!filters.destination||filters.destination==='Todos'||lead.destination===filters.destination)&&(!filters.status||filters.status==='Todos'||lead.status===filters.status)&&(!filters.outcome||filters.outcome==='Todos'||lead.commercialOutcome===filters.outcome)); }
...
export const supabaseBaseRepository: BaseRepository={
  ...
  async options(){const r=await all();const u=(v:string[])=>['Todos',...Array.from(new Set(v.filter(Boolean))).sort((a,b)=>a.localeCompare(b,'pt-BR'))];return{origins:u(r.map(x=>x.origin)),branches:u(r.map(x=>x.branch)),states:u(r.map(x=>x.state)),cities:u(r.map(x=>x.city)),destinations:u(r.map(x=>x.destination)),statuses:u(r.map(x=>x.status)),outcomes:u(r.map(x=>x.commercialOutcome||'Sem resultado'))};},
  ...
  async listByIds(ids){const set=new Set(ids);return(await all()).filter(r=>set.has(r.id));},
  async updateMetadata(id,outcome,notes){const response=await getSupabaseClient().rpc('update_permanent_record_metadata',{p_canonical_lead_id:Number(id),p_commercial_outcome:outcome==='Sem resultado'?'':outcome,p_operator_notes:notes});if(response.error)throw new Error(`Não foi possível atualizar o resultado comercial: ${response.error.message}`);},
  async compareAndArchive(id,expectedStatus){const response=await getSupabaseClient().rpc('archive_permanent_record',{p_canonical_lead_id:Number(id),p_expected_status_id:expectedStatus});if(response.error)throw new Error(`Não foi possível arquivar o registro permanente: ${response.error.message}`);if(!response.data)return null;return (await all()).find(r=>r.id===id)??null;},
};

...
export async function listChatChips(organizationId) {
    ...
    const chipsResult = await client.from('chips')
        ...
        .order('chips_name');
    if (chipsResult.error)
        throw new Error(chipsResult.error.message);
    ...
    const instances = new Map((instanceResult.data ?? []).map((item) => [id(item.instances_id), item]));
...
export async function listConversationUnreadCounts(organizationId) {
...
function meaningfulContactName(value) {
    ...
    if (/^[+\d\s().-]{7,}$/.test(candidate))
        return '';
    if (/^\d+@(?:s\.whatsapp\.net|c\.us|lid)$/i.test(candidate))
        return '';
    return candidate;
}
export async function listConversations(organizationId, chipId, includeArchived = false, limit = 120) {
    const client = getSupabaseClient();
    let query = client.from('conversations')
        ...
        .limit(limit);
    if (chipId)
        query = query.eq('chips_id', Number(chipId));
    if (!includeArchived)
    ...
    const leads = new Map((leadsResult.data ?? []).map((lead) => [id(lead.leads_id), lead]));
    return conversationRows.map((item) => {
        ...
        return {
            ...
            lastMessageDirection: ['inbound', 'outbound'].includes(text(item.last_message_direction)) ? text(item.last_message_direction) : null,
            updatedAt: text(item.conversations_updated_at),
        };
    });
}
export function mapConversationMessageRow(item) {
    return {
        ...
        status: text(item.message_status),
...
export async function listConversationMessages(organizationId, conversationId, limit = 80, beforeId = null) {
    ...
    let query = getSupabaseClient().from('conversation_messages')
        ...
        .limit(limit);
    if (beforeId && Number.isSafeInteger(Number(beforeId)))
        query = query.lt('conversation_messages_id', Number(beforeId));
    const result = await query;
    if (result.error)
        throw new Error(result.error.message);
    return sortConversationMessages((result.data ?? []).map(mapConversationMessageRow));
}

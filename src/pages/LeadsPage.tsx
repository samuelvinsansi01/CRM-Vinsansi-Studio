...
import { COMMERCIAL_STAGE_LABELS, type CrmLead } from '../services/leads/crmLead.types';
...
export function LeadsPage() {
  ...
  const filters = useMemo(() => ({
    ...
    branchId,
    state,
  }), [branchId, channel, debouncedSearch, state, statusId]);
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      await refresh();
      pushToast({
        title: wasNoContact ? 'Empresa recuperada' : 'Empresa atualizada',
        description: wasNoContact
          ? 'O contato foi salvo e a empresa voltou para Importado, pronta para entrar novamente na operação.'
          : 'Os dados canônicos da empresa foram atualizados.',
        tone: 'success',
      });
    } catch (err) {
  ...
  const confirmInvalidate = async () => {
    ...
    try {
      ...
      setPage(1);
      await refresh();
      pushToast({ title: 'Empresa invalidada', description: 'A empresa foi marcada como inválida e não será puxada para uma nova fila.', tone: 'success' });
    } catch (err) {
  ...
  const confirmReturn = async () => {
    ...
    try {
      ...
      setPage(1);
      await refresh();
      pushToast({ title: 'Empresa retornada', description: 'A empresa voltou para Importado e pode entrar novamente na operação.', tone: 'success' });
    } catch (err) {
  ...
  const addManualLead = async () => {
    ...
    try {
      ...
      setPage(1);
      await refresh();
      pushToast({ title: 'Empresa inserida', description: 'A empresa entrou na base como Importado e já pode ser preparada para envio.', tone: 'success' });
    } catch (err) {
  ...
  return (
    <div className="dashboard-table-page lead-list-page crm-leads-page">
      <PageHeader
        title="Empresas"
        description="Base permanente e canônica de todas as empresas que já passaram pela operação. Comercial e Projetos são apenas visões de fases do mesmo registro."
        action={(
          <div className="page-header-actions">
            ...
            {canCreate ? <Button iconLeft={Plus} onClick={() => setInsertOpen(true)}>Inserir empresa</Button> : null}
          </div>
        )}
      />
      ...
      >
        ...
        {!error && loading ? <div className="table-message">Carregando empresas...</div> : null}
        {!error && !loading && !rows.length ? <div className="table-message">Nenhuma empresa encontrada.</div> : null}
      ...
      <Drawer
        ...
        description="Cadastro manual na base permanente. Estado e cidade usam os catálogos canônicos do CRM e a empresa entra como Importado."
        onClose={() => { if (!saving) { setInsertOpen(false); setManual(EMPTY_MANUAL); setCityOptions([]); } }}
        footer={<><Button variant="secondary" disabled={saving} onClick={() => { setInsertOpen(false); setManual(EMPTY_MANUAL); setCityOptions([]); }}>Cancelar</Button><Button loading={saving} onClick={() => void addManualLead()}>Inserir empresa</Button></>}
      >
      ...
      <Drawer
        ...
        description={editingLead?.statusId === LEAD_STATUS.NO_CONTACT
          ? 'Ao salvar um contato válido, esta empresa sai de Sem contato e volta automaticamente para Importado.'
          : editingLead?.statusId === LEAD_STATUS.INVALID
            ? 'Editar não reativa uma empresa inválida. Depois de corrigir os dados, use Retornar para Importado.'
            : 'Atualize os dados canônicos da empresa. Itens em revisão/fila continuam sendo controlados em Fila de Disparo.'}
        onClose={closeEdit}
      ...
      <Drawer open={viewingLead !== null} title="Detalhes da empresa" description="Consulta do registro canônico da empresa." onClose={() => setViewingLead(null)} footer={<Button variant="secondary" onClick={() => setViewingLead(null)}>Fechar</Button>}>
      ...
      <ConfirmDialog
        ...
        title="Invalidar empresa?"
        description="A empresa será marcada como Inválida e ficará separada de Sem contato."
      ...
      <ConfirmDialog
        ...
        description="A empresa voltará para a operação. O canal será recalculado com base nos contatos atuais."

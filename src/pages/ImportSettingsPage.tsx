...
function cloneSettings(settings: ImportSettings): ImportSettings {
  return {
    ...settings,
    ...
    logs: { ...settings.logs },
    branchRules: settings.branchRules.map((rule) => ({ ...rule, subcategories: [...rule.subcategories] })),
  };
}
...
export function ImportSettingsPage() {
  ...
  const save = async () => {
    if (!draft) return;
    try {
      const nextSettings = await updateSettings(draft);
      setDraft(cloneSettings(nextSettings));
  ...
  return (
    <div className="settings-page import-settings-page">
      ...
      <p className="settings-note">Os campos ficam em rascunho nesta tela. A importação só usa estes valores depois de clicar em Salvar.</p>
      ...
      <section className="settings-grid import-settings-grid">
        <Panel title="Critérios mínimos" className="settings-card import-settings-card">
          ...
          <BooleanSetting label="Modo simulação" description="Executa validações e relatório sem gravar no banco. Recomendado para testar regras." value={getBoolean(draft, 'safeMode.simulationMode')} onChange={(value) => updateBoolean('safeMode.simulationMode', value)} />
        ...
        <Panel title="Deduplicação" className="settings-card import-settings-card">
          ...
          <BooleanSetting label="Bloquear Base Permanente" description="Impede reimportar contatos já enviados/arquivados na base." value={getBoolean(draft, 'deduplication.blockBasePermanent')} onChange={(value) => updateBoolean('deduplication.blockBasePermanent', value)} />
          <BooleanSetting label="Bloquear sent_contacts" description="Impede reimportar contatos já disparados no sistema antigo." value={getBoolean(draft, 'deduplication.blockSentContacts')} onChange={(value) => updateBoolean('deduplication.blockSentContacts', value)} />

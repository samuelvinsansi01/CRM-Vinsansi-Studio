...
export function HomologationPage(){
  ...
  const summary=useMemo(()=>snapshot.checks.reduce<{total:number;pending:number;passed:number;failed:number;not_applicable:number}>((acc,item)=>{acc.total++;acc[item.status]++;return acc;},{total:0,pending:0,passed:0,failed:0,not_applicable:0}),[snapshot.checks]);

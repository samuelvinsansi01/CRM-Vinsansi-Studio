...
type PromotionForm={name:string;alternativeName:string;instagram:string;website:string;mapsUrl:string;branchId:string;countryId:string;stateId:string;cityId:string;contactSourceId:string};
const emptyPromotionForm:PromotionForm={name:'',alternativeName:'',instagram:'',website:'',mapsUrl:'',branchId:'',countryId:'',stateId:'',cityId:'',contactSourceId:''};
...
export function ConversationsPage() {
  ...
  const selectedConversationPhone = String(selectedConversation?.phone ?? '').trim();
  ...
  const openPromotionDrawer=async()=>{
    ...
    try{
      ...
      setPromotionForm({name:'',alternativeName:'',instagram:'',website:'',mapsUrl:'',branchId:'',countryId:brazil?.id??'',stateId:'',cityId:'',contactSourceId:source?.id??''});
  ...
  const handlePromoteContact=async()=>{if(!organizationId||!selectedConversation||promotionSaving)return;if(!selectedConversationPhone){setError('O contato ainda não possui um WhatsApp telefônico canônico. Aguarde a identificação do número antes de cadastrar como lead.');return;}setPromotionSaving(true);try{await promoteConversationContact(organizationId,selectedConversation.id,{name:promotionForm.name,alternativeName:promotionForm.alternativeName||undefined,instagram:promotionForm.instagram||undefined,website:promotionForm.website||undefined,mapsUrl:promotionForm.mapsUrl||undefined,branchId:Number(promotionForm.branchId),countryId:Number(promotionForm.countryId),stateId:promotionForm.stateId?Number(promotionForm.stateId):null,cityId:promotionForm.cityId?Number(promotionForm.cityId):null,contactSourceId:Number(promotionForm.contactSourceId)});const delta=await getConversationDelta(organizationId,selectedConversation.id);if(delta)setConversations((current)=>current.map((item)=>item.id===delta.id?delta:item));void loadCommercial(selectedConversation.id,true);setPromotionOpen(false);toast({title:'Lead criado',description:'A conversa e o histórico foram preservados e agora estão vinculados à empresa cadastrada.',tone:'success'});}catch(cause){setError(cause instanceof Error?cause.message:'Falha ao cadastrar contato como lead.');}finally{setPromotionSaving(false);}};
  ...
  return (
    <div className="chat-page">
      ...
      <Drawer open={promotionOpen} title="Cadastrar como lead" description="O contato canônico, a conversa e todo o histórico serão preservados. O nome recebido no WhatsApp é apenas uma referência; informe a empresa canônica abaixo." onClose={()=>!promotionSaving&&setPromotionOpen(false)} footer={<><Button variant="secondary" disabled={promotionSaving} onClick={()=>setPromotionOpen(false)}>Cancelar</Button><Button loading={promotionSaving} disabled={!promotionForm.name.trim()||!selectedConversationPhone||!promotionForm.branchId||!promotionForm.countryId||!promotionForm.contactSourceId} onClick={()=>void handlePromoteContact()}>Cadastrar lead</Button></>}>
        <div className="drawer-form">
          <Field label="Nome recebido no WhatsApp" value={selectedConversation?.contactName||'—'} readOnly/>
          <Field label="Empresa *" value={promotionForm.name} placeholder="Nome canônico da empresa" maxLength={160} onChange={(name)=>setPromotionForm((v)=>({...v,name}))}/>
          <Field label="Nome alternativo (opcional)" value={promotionForm.alternativeName} placeholder="Opcional" maxLength={160} onChange={(alternativeName)=>setPromotionForm((v)=>({...v,alternativeName}))}/>
          <Field label="WhatsApp *" value={selectedConversation?.phone||''} readOnly/>
          <Field label="Instagram (opcional)" value={promotionForm.instagram} placeholder="@empresa ou URL" onChange={(instagram)=>setPromotionForm((v)=>({...v,instagram}))}/>
          <Field label="Site (opcional)" value={promotionForm.website} placeholder="https://empresa.com.br" onChange={(website)=>setPromotionForm((v)=>({...v,website}))}/>
          <Field label="Google Maps (opcional)" value={promotionForm.mapsUrl} placeholder="https://maps.google.com/..." onChange={(mapsUrl)=>setPromotionForm((v)=>({...v,mapsUrl}))}/>
          <label className="drawer-field"><span>Ramo *</span><SelectField value={promotionForm.branchId} searchable searchPlaceholder="Buscar ramo..." options={promotionBranches.map((x)=>({value:x.id,label:x.name}))} placeholder="Selecione" onChange={(branchId)=>setPromotionForm((v)=>({...v,branchId}))}/></label>
          <label className="drawer-field"><span>País *</span><SelectField value={promotionForm.countryId} searchable searchPlaceholder="Buscar país..." options={promotionCountries.map((x)=>({value:x.id,label:x.name}))} placeholder="Selecione" onChange={(countryId)=>setPromotionForm((v)=>({...v,countryId,stateId:'',cityId:''}))}/></label>
          <label className="drawer-field"><span>Estado (opcional)</span><SelectField value={promotionForm.stateId} searchable searchPlaceholder="Buscar estado..." options={promotionStates.map((x)=>({value:x.id,label:x.name}))} placeholder="Opcional" onChange={(stateId)=>setPromotionForm((v)=>({...v,stateId,cityId:''}))}/></label>
          <label className="drawer-field"><span>Cidade (opcional)</span><SelectField value={promotionForm.cityId} searchable searchPlaceholder="Buscar cidade..." options={promotionCities.map((x)=>({value:x.id,label:x.name}))} placeholder="Opcional" onChange={(cityId)=>setPromotionForm((v)=>({...v,cityId}))}/></label>
          <label className="drawer-field"><span>Origem *</span><SelectField value={promotionForm.contactSourceId} searchable searchPlaceholder="Buscar origem..." options={promotionSources.map((x)=>({value:x.id,label:x.name}))} placeholder="Selecione" onChange={(contactSourceId)=>setPromotionForm((v)=>({...v,contactSourceId}))}/></label>
        </div>
      </Drawer>
